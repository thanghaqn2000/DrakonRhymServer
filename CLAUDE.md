...
- **Pitch shift formula**: `pitch_factor = 2 ** ((semitones*100 + cents) / 1200)`, applied as the FFmpeg filter chain `asetrate={sr*factor},aresample={sr}`. This shifts pitch and tempo together — that is intentional and matches the spec. If true pitch-only shifting is requested later, swap in the `rubberband` filter (requires an FFmpeg build linked against `librubberband` — the slim Docker image does not have it by default).
